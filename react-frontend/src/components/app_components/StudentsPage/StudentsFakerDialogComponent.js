...
const StudentsFakerDialogComponent = (props) => {
    ...
    const [studentIDIds, setstudentIDIds] = useState();


    useEffect(() => {
        setFakerCount(1);
       
        client.service("studentsGrades")
                        .find({ query: { $select: ["_id","studentId"] } })
                        .then((data) => {
                        setstudentIDIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = studentsFakerFactory(props.user,fakerCount,studentIDIds);
